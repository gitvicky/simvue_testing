...
"""
# %%
configuration = {"Case": 'Turbulent & Laminar & Laminar_Steady',
                 "Field": 'U',
                 "Type": '2D Time',
                 "Epochs": 500,
                 "Batch Size": 5,
                 ...
                 "Scheduler Gamma": 0.5,
                 "Activation": 'GELU',
                 "Normalisation Strategy": 'Unit Gaussian',
...
INPUTS = ['FNO.py']
OUTPUTS = ['FNO_turb_laminar.pth', 'FNO_mixed_turbulent.png',  'FNO_mixed_laminar.png', 'FNO_mixed_laminar_steady.png']
...
# %%
data1 =  np.load(os.path.dirname(os.getcwd()) + '/turbulent/NS_turbulent.npz')
data2 =  np.load(os.path.dirname(os.getcwd()) + '/laminar/NS_laminar.npz')
data3 =  np.load(os.path.dirname(os.getcwd()) + '/laminar_steady/NS_laminar_steady.npz')
# %%
ntrain = 135
ntest = 15
...
#Training Data
if field == 'U':
    u_sol = np.vstack((data1['U'].astype(np.float32)[:45], data2['U'].astype(np.float32)[:45], data3['U'].astype(np.float32)[:45]))
elif field == 'V':
    u_sol = np.vstack((data1['V'].astype(np.float32)[:45], data2['V'].astype(np.float32)[:45], data3['V'].astype(np.float32)[:45]))
if field == 'P':
    u_sol = np.vstack((data1['P'].astype(np.float32)[:45], data2['P'].astype(np.float32)[:45]), data3['P'].astype(np.float32)[:45])

x = data1['x'].astype(np.float32)
y = data1['y'].astype(np.float32)
t = data1['t'].astype(np.float32)
...
#Testing Data 
if field == 'U':
    u_sol = np.vstack((data1['U'].astype(np.float32)[-5:], data2['U'].astype(np.float32)[-5:], data3['U'].astype(np.float32)[-5:]))
elif field == 'V':
    u_sol = np.vstack((data1['V'].astype(np.float32)[-5:], data2['V'].astype(np.float32)[-5:], data3['V'].astype(np.float32)[-5:]))
if field == 'P':
    u_sol = np.vstack((data1['P'].astype(np.float32)[-5:], data2['P'].astype(np.float32)[-5:], data3['P'].astype(np.float32)[-5:]))
...
torch.save(model.state_dict(), 'FNO_turb_laminar.pth')
...
plt.savefig('FNO_mixed_turbulent.png')
...
plt.savefig('FNO_mixed_laminar.png')
...
plt.savefig('FNO_mixed_laminar_steady.png')
# %%
