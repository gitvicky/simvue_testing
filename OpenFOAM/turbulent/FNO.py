...
INPUTS = ['FNO.py']
OUTPUTS = ['FNO_turbulent.pth', 'FNO_turbulent.png']

run = Simvue()

run.init(folder="/HPC-AI/turbulent", tags=['OpenFOAM', 'Turbulent', 'Surrogate', 'FNO'], metadata=configuration)
...
# %%
data =  np.load(os.getcwd()+ '/NS_turbulent.npz')
...
torch.save(model.state_dict(), 'FNO_turbulent.pth')
...
run.update_metadata({'Training Time': train_time,
                     'MSE Test Error': MSE_error,
                     'MAE Test Error': MAE_error,
                     'LP Test Error': LP_error
                    })
