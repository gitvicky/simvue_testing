...
def run_sim(ii, D_damp, c, mu, sigma):
    ...
    run = wandb.init(project='1D-ConvDiff',
                    notes='',
                    config=run_params,
                    mode='online')
    ...
    wandb.save(os.getcwd() + '/Inputs/' + run_name + '_u0.npy')
    ...
    wandb.save(os.getcwd() + '/Images/' + run_name + '.png')

    u_dataset = u_dataset[::50]
    
    np.save(os.getcwd() + '/Outputs/' + run_name + '_u.npy', u_dataset)
    wandb.save(os.getcwd() + '/Outputs/' + run_name + '_u.npy')
